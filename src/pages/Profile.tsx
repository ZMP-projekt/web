...
export const Profile: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div
                className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-3xl p-8 relative overflow-hidden flex flex-col md:flex-row items-center gap-8">
                <div
                    className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl pointer-events-none"></div>

                <div
                    className="w-32 h-32 rounded-full bg-slate-700 border-4 border-slate-800 overflow-hidden flex items-center justify-center relative z-10 shrink-0 shadow-xl">
                    {formData.photoUrl ? (
                        <img
                    ...
                    <div className="z-10 shrink-0 mt-4 md:mt-0">
                        {!isEditing ? (
                            <button onClick={() => setIsEditing(true)}
                                    className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-white px-5 py-2.5 rounded-xl font-bold transition-colors">
                            ...
                            <button onClick={() => {
                                ...
                                setFormData(profile || formData);
                                    ...
                                    className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-600 px-5 py-2.5 rounded-xl font-bold transition-colors">
            ...
            {error && (
                <div
                    className="flex items-center gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 animate-in fade-in">
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-3xl p-8">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <InputField label={t('common:first_name')} name="firstName" value={formData.firstName}
                                isEditing={isEditing} onChange={handleChange} />
                    <InputField label={t('common:last_name')} name="lastName" value={formData.lastName}
                                isEditing={isEditing} onChange={handleChange} />
                        ...
                        <>
                            {isEditing && (
                                <div className="md:col-span-2">
                                    <label
                                        className="block text-sm font-bold text-slate-400 mb-2 uppercase tracking-wider">
                                        {t('photo_url')}
                                    </label>
                            ...
                            <div className="md:col-span-2">
                                <InputField label={t('specialization')} name="specialization"
                                            value={formData.specialization || ''} isEditing={isEditing}
                                            onChange={handleChange} />
                            </div>

                            <div className="md:col-span-2">
                                <label
                                    className="block text-sm font-bold text-slate-400 mb-2 uppercase tracking-wider">{t('bio')}</label>
                                {isEditing ? (
                                    <textarea
                                        name="bio"
                                        value={formData.bio || ''}
                                    ...
                                    <div
                                        className="w-full px-4 py-3 bg-slate-900/20 border border-slate-700/50 rounded-xl text-slate-300 min-h-24 leading-relaxed">
...
const InputField = ({ label, name, value, isEditing, onChange }: {
    ...
    <div>
        ...
        {isEditing ? (
            ...
            <div
                className="w-full px-4 py-3 bg-slate-900/20 border border-slate-700/50 rounded-xl text-white font-medium">
